...
// Code By Tripwire Interactive

class daZombieCrawler extends ZombieCrawlerBase;
...
event Bump(actor Other)
{
	...
	if(bPouncing && da_players_ClassPlayer(Other)!=none )
	{
		da_players_ClassPlayer(Other).TakeDamage(((MeleeDamage - (MeleeDamage * 0.05)) + (MeleeDamage * (FRand() * 0.1))), self ,self.Location,self.velocity, class 'KFmod.ZombieMeleeDamage');
		if (da_players_ClassPlayer(Other).Health <=0)
		{
			//TODO - move this to humanpawn.takedamage? Also see KFMonster.MeleeDamageTarget
			da_players_ClassPlayer(Other).SpawnGibs(self.rotation, 1);
		}
...
defaultproperties
{
     ...
     ControllerClass=Class'DA2.daCrawlerController'
}
