class daCrawlerController extends KFMonsterController;
...
function bool IsInPounceDist(actor PTarget)
{
  ...
  time = vsize(DistVec)/daZombieCrawler(pawn).PounceSpeed;
  ...
  if((abs(EndHeight - PTarget.Location.Z) < Pawn.CollisionHeight + PTarget.CollisionHeight + 300) &&
    ...
    else
    {
        ...
        if((LastPounceTime + (4.5 - (FRand() * 3.0))) < Level.TimeSeconds )
        {
            ...
            if ( RelativeDir > 0.85 )
            {
                ...
                if(IsInPounceDist(A) )
                {
                    if(daZombieCrawler(Pawn).DoPounce()==true )
...
{
  if( dazombiecrawler(pawn).bPouncing )
  {
